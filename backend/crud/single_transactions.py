...
def get_single_transactions_by_warehouse(db: Session, warehouse_id: int, skip: int = 0, limit: int = 100) -> List[SingleTransaction]:
    return db.query(SingleTransaction).filter(SingleTransaction.warehouse_id == warehouse_id).offset(skip).limit(limit).all()


def get_single_transactions_by_type(db: Session, transaction_type: str, skip: int = 0, limit: int = 100) -> List[SingleTransaction]:
    return db.query(SingleTransaction).filter(SingleTransaction.type == transaction_type).offset(skip).limit(limit).all()


def create_single_transaction(db: Session, transaction: SingleTransactionCreate) -> SingleTransaction:
    db_transaction = SingleTransaction(**transaction.model_dump())
